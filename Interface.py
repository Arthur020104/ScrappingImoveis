...
class Interface:
    ...
    def setup_tab3(self):
        self.title3 = ttk.Label(self.tab3, text="Interface - AREA TERRITORIAL", font=('Arial', 24))
        self.title3.pack(pady=10)
        ...
        self.is_using_complete_code_var = tk.BooleanVar()
        self.is_using_complete_code_checkbutton = ttk.Checkbutton(self.tab3, text="Usar Código Completo", variable=self.is_using_complete_code_var, command=self.toggle_parte_entry_pdf)

        self.entry_parte_pdf = ttk.Entry(self.tab3)
        self.entry_parte_pdf.insert(0, "Parte a ser Mudada")
    ...
    def show_in_order(self):
        ...
        if self.use_complete_code:
            self.entry_parte_pdf.pack(pady=5)
        self.is_using_complete_code_checkbutton.pack(pady=5)

        
        self.update_base_checkbutton_pdf.pack(pady=10)

        if not self.update_base:
    ...
    def toggle_parte_entry_pdf(self):
        self.use_complete_code = not self.use_complete_code
        self.show_in_order()

    def toggle_base_path_entry_pdf(self):
        self.update_base = not self.update_base
        self.show_in_order()

    def request_data_from_pdf(self):
        try:
            start_code = self.entry_start_code_pdf.get()
            ...
            parte = int(self.entry_parte_pdf.get()) if self.is_using_complete_code_var.get() else None
            is_using_complete_code = self.is_using_complete_code_var.get()

            if self.update_base_var_pdf.get():
                request_new_pdf_data(start_code, end_code, records_per_chunk, concurrent_chunks, parte, delete_file=True, is_using_complete_code=is_using_complete_code)
                ...
                new_file, status = request_new_pdf_data(start_code, end_code, records_per_chunk, concurrent_chunks, parte, delete_file=False, is_using_complete_code=is_using_complete_code)
                if not new_file:
...
if __name__ == '__main__':
    ...
    img_path = os.path.join(os.path.dirname(__file__), "Images/letter-g.png")
