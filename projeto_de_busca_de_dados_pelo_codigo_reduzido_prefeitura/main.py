...
import get_cod_completo  # Importa um módulo customizado para obter códigos e endereços
...
def process_chunks(start_code:int, end_code:int, records_per_chunk:int, concurrent_chunks:int):
    ...
    Args:
        ...
        records_per_chunk (int): Número de dados a serem salvos por chunk
        concurrent_chunks (int): Número de chunks processados simultaneamente
    
    return: combined_output.csv path
    """
    start_time = time.time()
    
    # Define a faixa de códigos únicos a partir dos parâmetros inicial e final
    code_range = range(start_code, end_code)

    # Calcula o tamanho desejado dos chunks com base no número de dados a serem salvos por chunk e o número de chunks simultâneos
    chunk_size = records_per_chunk / concurrent_chunks

    # Calcula o número de chunks que serão processados simultaneamente
    total_chunks = int(len(code_range) / chunk_size)
    ...
    def process_chunk(chunk, result_list):
        ...
        result_list.extend(get_cod_completo.get_property_number_and_adress(chunk))

    # Ajusta o tamanho dos chunks para garantir o tamanho desejado
    chunk_size = int(len(code_range) / total_chunks)
    chunk_size = chunk_size + 1 if len(code_range) % total_chunks != 0 else chunk_size
    chunks = [code_range[i:i + chunk_size] for i in range(0, len(code_range), chunk_size)]
